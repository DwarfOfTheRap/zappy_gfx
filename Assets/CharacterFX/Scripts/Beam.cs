...
public class Beam : MonoBehaviour {
	...
	private IEnumerator doBeamOut(bool Destroy)
	{
		...
		while (LengthLeft > 0.0f)
    	{
        	...
        	yield return null;
			LengthLeft -= Time.deltaTime * 2.5f * GameManagerScript.instance.timeManager.timeSpeed;
    	}
	...
	private IEnumerator doBeamIn()
	{
		...
		while (LengthLeft > 0.0f)
    	{
			...
			SetMaterialParms(pos);
        	yield return null;
			LengthLeft -= Time.deltaTime * 1.5f * GameManagerScript.instance.timeManager.timeSpeed;
    	}
